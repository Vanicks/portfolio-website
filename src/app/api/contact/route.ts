...
export async function POST(request: Request) {
  ...
  const {
    ...
    name,
    message,
  }: { email: string; name: string; message: string } = await request
    .clone()
    .json();
  ...
  const embed = {
    title: '📩 New message from paul.is-a.dev',
  ...
  try {
    const response = await fetch(
      `${process.env.DISCORD_WEBHOOK_URL}?wait=true`,
      {
        ...
        body: JSON.stringify({
          username: 'kaskus',
          avatar_url: 'https://paul.is-a.dev/favicon.ico',
          embeds: [embed],
        }),
      },
    );
    ...
    if (!data.id) {
      return new Response(
        JSON.stringify({ error: true, message: 'Unable to send message' }),
        {
          status: 500,
          headers: {
            'Content-Type': 'application/json',
